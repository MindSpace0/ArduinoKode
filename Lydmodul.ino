...
const int synthIn = 7;    //definert slik at den kan ta imot signal fra den andre modulen
...
void setup(void) {
  ...
  pinMode(synthIn,INPUT_PULLUP);  // definerer pin som input
}

void loop(void) {
  if (digitalRead(synthIn)==HIGH);  // Denne if-løkken la vi til for å kun kjøre funksjonen MIDI.read() 
   MIDI.read();     // hvis / når den får signal fra den andre modulen
 }
